...
<script>/* inject-assets.mjs 注入通用文案编辑运行时 */</script>
...
<nav class="proto-nav">
  ...
  <span class="proto-title" data-proto-edit="prototype.title">{页面名}</span>
</nav>

<!-- 三栏布局：左 TOC（sticky 卡片）+ 中原型纵列 + 右功能概览（每个页面 section 内 sticky） -->
...
<!-- 若输入 PRD 有流程图或核心用户流程，在第一个页面 section 前加入。它不对应设备页面。 -->
<div id="flow-overview" class="flow-stack">
  <div class="section-label">F-01 流程总览</div>
  <section class="flow-overview">
    <!-- 用 SVG 绘制流程。页面节点用 <a href="#section-xxx"> 包裹；系统节点不跳转。 -->
  </section>
</div>
  ...
  <div class="proto-with-prd">
    ...
    <div class="macos-window">
      <div class="macos-titlebar">
        ...
        <div class="macos-title"><span data-proto-edit="app.name">{App Name}</span> — <span data-proto-edit="window.title">{窗口标题}</span></div>
      </div>
    ...
    <div class="prd-panel-wrap">
      <aside class="prd-panel">
        <div class="prd-panel__title">功能概览 · PRD</div>
